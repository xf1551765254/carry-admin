...
<script>
  import commonTree from '@/components/carry/CommonTree.vue'
    ...
    computed: {
      menuComponent() {
        ...
        if (this.menu.path && this.menu.path !== '/') {
          ...
          comp = `组件路径：src/views/carry/Index.vue`
        }
        return comp
      }
    },
    ...
    methods: {
      ...
      computedComponent() {
        ...
        if (isUrl) {
          ...
          if (this.menu.path) {
            this.menu.component = `carry${this.menu.path}/Index`
          } else {
            this.menu.component = `carry/Index`
          }
        }
      },
      initMenuTree() {
